...
import { PH_CACHE_FNAME, PH_CONFIG_FNAME } from '../env'
...
export const getProjectRoot = () => {
  const root = findUpSync(PH_CONFIG_FNAME, {})
  if (!root) return cwd()
  return dirname(root)
}

export const localStorage = new LocalStorage(
  join(getProjectRoot(), PH_CACHE_FNAME)
)

export const getProject = (): Partial<ProjectConfig> => {
  const path = join(getProjectRoot(), PH_CONFIG_FNAME)
